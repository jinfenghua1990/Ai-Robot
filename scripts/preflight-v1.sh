...
chk_v2_attached()  { cd "$REPO" && "$PY" -c 'import backend.main as m; routes=list(m.app.router.routes); embedded=[i for i,r in enumerate(routes) if getattr(r,"path","")=="/api/v2/health" and getattr(getattr(r,"endpoint",None),"__module__","").startswith("v2_app.")]; proxy=[i for i,r in enumerate(routes) if getattr(r,"path","")=="/api/v2/{full_path:path}"]; assert embedded, "9000 main app missing embedded /api/v2/health"; assert not proxy or embedded[0] < proxy[0], f"legacy V2 proxy precedes embedded routes: embedded={embedded[0]} proxy={proxy[0]}"; print(f"9000 embedded V2 attached at route {embedded[0]}" + (f", proxy fallback after it at {proxy[0]}" if proxy else ", no proxy fallback"))'; }
...
run_check v2_attached      "9000 主应用实际挂载内嵌 V2，且优先于旧代理"    chk_v2_attached
