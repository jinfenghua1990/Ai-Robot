...
class QuantPipeline:
    ...
    def run(self, history: Mapping[str, Sequence[DailyBar]], trade_date: date, market: MarketContext) -> list[SignalSnapshot]:
        ...
        values = self.factors.calculate(universe, trade_date)
        return self._score_values(values, trade_date)
    ...
    def run_with_values(self, history: Mapping[str, Sequence[DailyBar]], trade_date: date, market: MarketContext):
        ...
        universe = build_universe(history, trade_date, market)
        values = self.factors.calculate(universe, trade_date)
        return values, self._score_values(values, trade_date)

    def _score_values(self, values, trade_date: date):
        ...
        for ts_code, dimensions in scores.items():
            ...
            factor_score = self.scorer.factor_score(dimensions)
            lifecycle = lifecycle_state(dimensions)
            state = trading_state(dimensions, resonance.eligible)
            reasons = [f"resonance={resonance.count}", f"lifecycle={lifecycle}", f"state={state}"]
            snapshots.append(SignalSnapshot(ts_code, trade_date, factor_score, dimensions, resonance, lifecycle, state, reasons))
