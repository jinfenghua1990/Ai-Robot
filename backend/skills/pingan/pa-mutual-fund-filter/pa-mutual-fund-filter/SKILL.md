---
name: pa-mutual-fund-filter
description: 查询和筛选场外公募基金排行榜数据。覆盖两类榜单：一是按单一指标排序的通用榜单（阶段收益、七日年化、最大回撤、波动率、夏普比率、卡玛比率、最长套牢天数、持有期盈利概率），对应收益榜、稳健榜、低回撤榜、高胜率榜、上涨先锋等场景；二是平台特色榜单（热销榜、人气榜、定投榜）。支持按基金类型、风险等级、规模、成立年限、申购费率及金牛奖/五星评级/高股息等标签筛选。当用户询问"某类基金收益排行""最近好买/热销/申购最火的基金""稳健理财基金推荐""回撤小/波动低/风险低的基金""夏普或卡玛比率高的基金""长期胜率高的基金""货币基金七日年化排行""金牛奖/五星评级基金""基金人气榜/定投榜"等场外基金筛选问题时，使用此 skill 实时查询数据，不要依赖训练知识回答基金排名、收益率等具体数字。当用户查询场内 ETF（在交易所直接买卖的 ETF）时不要触发此 skill，应改用场内 ETF 筛选技能；但 ETF 联接基金属于场外基金，仍由本 skill 处理。查询单只股票行情、资金流向、K线等非基金榜单问题时也不要触发此 skill。
...
`--firstITypes` 取值：股票型、混合型、债券型、货币型、QDII、指数型、ETF（指 ETF 联接，场外)、商品型基金、REITs、股票多空、FOF
