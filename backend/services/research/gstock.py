...
from services.research import astock
...
def get_klines(symbol: str, days: int = 252) -> list[dict] | None:
    """获取美股/港股历史K线（东财 push2his）
    ...
    params = {
        ...
        "fqt": "1",     # 前复权
        "end": "20500101",
        "lmt": str(min(days, 1000)),   # 最多1000天
    }
    try:
        r = astock.em_get(url, params=params, headers=_UA_H, timeout=15)
        d = r.json()
        klines = d.get("data", {}).get("klines", [])
    except Exception as exc:
        import logging
        logging.getLogger(__name__).debug(f"[gstock] get_klines failed {symbol}: {exc}")
        return None
