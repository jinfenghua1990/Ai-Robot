...
export function DailyReview() {
  ...
  const addWatch = async () => {
    ...
    setWatchInput("");
    if (!added) return;
    setWatchCodes(next); await saveWatch(next); refreshWatch(next);
  };
  ...
  return (
    <div>
      ...
      <GlassCard className="mb-6">
        ...
        {watchCodes.length === 0 ? (
          <p className="text-sm text-muted-foreground/60">加上你关注的股票，随时看它们的实时价格与涨跌。数据写入 AIROBOT 共享自选股，各子系统同步。</p>
        ) : (
