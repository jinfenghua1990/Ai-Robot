...
import {
  Activity, Radar, LayoutGrid, Settings, Search, NotebookPen,
  Moon, Sun, ChevronsLeft, ChevronsRight, LineChart, Github, UserRound,
  Cog, Cpu, Database, Cable, Rocket, FlaskConical, FileText,
} from "lucide-react";
...
function useEmbedded() {
  ...
  useEffect(() => {
    ...
    setEmbedded(params.get("embedded") === "true" || window.self !== window.top);
  }, []);
  return embedded;
}

const NAV = [
  { to: `${BASE}/daily-review`, icon: Activity, label: "每日复盘" },
  { to: `${BASE}/intel`, icon: Radar, label: "资讯雷达" },
  { to: `${BASE}/sectors`, icon: LayoutGrid, label: "板块中心" },
  { to: `${BASE}/stock-data`, icon: Search, label: "个股数据" },
  { to: `${BASE}/my-reports`, icon: FileText, label: "我的研报" },
  { to: `${BASE}/notes`, icon: NotebookPen, label: "研究记录" },
  { to: `${BASE}/settings`, icon: Settings, label: "接入 AI" },
];

// 常看的板块，作为「板块中心」下的快捷入口（缩进显示）。
const SECTOR_LINKS = [
  { to: `${BASE}/sectors/humanoid`, icon: Cog, label: "人形机器人" },
  { to: `${BASE}/sectors/ai-computing`, icon: Cpu, label: "AI 算力" },
  { to: `${BASE}/sectors/hbm`, icon: Database, label: "HBM" },
  { to: `${BASE}/sectors/cpo`, icon: Cable, label: "光互联" },
  { to: `${BASE}/sectors/business-space`, icon: Rocket, label: "商业航天" },
  { to: `${BASE}/sectors/ai-pharma`, icon: FlaskConical, label: "生物医药" },
];
...
export function Layout() {
  ...
  return (
    <div className="flex h-screen">
      {/* Sidebar - 在 AIROBOT iframe 嵌入模式下隐藏，避免 UI 套 UI */}
      {!embedded && (
      <aside className={cn(
        ...
        <div className={cn("border-b border-border/50", collapsed ? "flex justify-center p-3" : "p-4")}>
          <Link to={`${BASE}/daily-review`} className={cn("flex items-center", collapsed ? "justify-center" : "gap-2")}>
      ...
      {/* Main */}
      <main className={cn("flex-1 overflow-auto", embedded && "w-full")}>
