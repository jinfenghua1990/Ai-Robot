...
export async function downloadReport(id: string, name: string): Promise<void> {
  const resp = await fetch(`/api/vibe/myreports/file/${id}`, { headers: authHeaders() });
...
async function request<T>(path: string, method: "GET" | "POST" | "DELETE" = "GET", body?: unknown): Promise<T> {
  ...
  try {
    resp = await fetch(`/api/vibe${path}`, opts);
  } catch {
    throw new ApiError("连接不到后端，请先启动 AIROBOT 后端（uvicorn main:app --port 9000）", 0);
  }
...
export interface Holding {
  ...
  market_value: number; pnl: number; pnl_pct: number; source?: string;
}
...
export const api = {
  ...
  announcementsBatch: (codes: string) => get<Record<string, Announcement[]>>(`/announcements/batch?codes=${codes}`),
  newsBatch: (codes: string) => get<Record<string, NewsItem[]>>(`/news/batch?codes=${codes}`),
